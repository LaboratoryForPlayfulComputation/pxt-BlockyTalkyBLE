...
void KeyValueService::send(String key, int type, Buffer value) {
    if (ble.getGapState().connected)
    {
        ...
        memcpy(&txCharacteristicMessage.key, PXT_STRING_DATA(key), min(BLOCKYTALKY_KV_KEY_LENGTH, PXT_STRING_DATA_LENGTH(key)));
        // write data type
        txCharacteristicMessage.type = type;
        // write value
        memcpy(&txCharacteristicMessage.value, PXT_STRING_DATA(value), min(BLOCKYTALKY_KV_VALUE_LENGTH, PXT_STRING_DATA_LENGTH(value)));
        // notify clients
